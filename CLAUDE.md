...
- Brand: `--red` `#FF4655`, `--cyan` `#00F0FF`, `--bg` `#0F1923`
...
**Inter is bundled locally** in `assets/fonts` so the app renders offline. It
ships weights 400 to 800 only. Do not reference a weight above 800 or a webfont
URL.
...
Two settled decisions, both from user feedback, that keep getting
reintroduced by accident:

- **No gradient backgrounds on panels.** Solid fills only. A panel that fades
  to white at the top was explicitly rejected.
